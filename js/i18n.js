...
(function () {
  ...
  <div class="help-tip">
    ...
    • <b>删线</b>：连线 →「{{eraser}} 删线模式」，点哪条删哪条<br>
...
<div class="modal-btns"><button class="btn primary" data-cmd="m-close">{{ok}}</button></div>`,
      ...
      // ---------------- 关于页 ----------------
      about_sub: "画人物关系连线的小工具 · 小红书离线版",
      about_date: "更新于 2026-09-10",
      about_sec_log: "📌 更新日志",
      about_sec_future: "🔮 未来前瞻",
      about_future: [
        "视图记忆：导出或切 Tab 后保留当前缩放与平移位置",
        "UI 布局重构：界面结构整体重排，操作更顺手",
      ...
      // ---------------- 布局说明（提示） ----------------
      layout_hint: "拖拽角色=换圈/沿圈移动；<b>槽位关</b>时可停在轨道任意角度（间距可以不规则，不会自动均分）；<b>槽位开</b>时自动吸附到就近空槽，多余空槽显示成虚线占位圈。想恢复整齐再点「平均排布」。拖动圈顶蓝点=改半径；点角色出现操作条。",
      ...
      toast_erase: "删线模式开：点连线两段式删除，点人物筛选其相关连线",

      // ---------------- 统一编辑器：批量录入（人物名单） ----------------
      ed_people_title: "批量录入",
      ed_people_hint: "粘贴名单（按圈号）或完整 CPC 文本——自动识别类型。增量合并，不覆盖已有内容。",
      ed_people_ph: "0：我\n1：甲，乙，丙\n2：丁，戊",
      ed_people_help: "<b>两种内容都收</b>，背后按类型自动路由：<br>· <b>纯名单</b>：<code>0：我</code>（圆心）、<code>1：甲，乙，丙</code>（第 1 圈）、<code>2：丁，戊</code>（第 2 圈）。分号「；」等同换行。<br>· <b>CPC 全文</b>：含 <code>#布局#</code> 段头即可；自动解析布局/喜好/连线/样式，同名合并。<br>改名 / 删人 / 加人，直接改文字后点「保存」。",

      // ---------------- 统一编辑器：逐条同步（连线 / CPC 五段语法） ----------------
      ed_link_title: "逐条同步",
      ed_link_hint: "参照格式说明进行数据编辑和添加。",
      ed_link_ph: "/* 数据解读格式见「CPChart 语言说明」 */\n#标题# 示例关系图\n#布局#\n0: 我\n1: 甲,乙,丙\n#喜好#\n本命: 甲,乙; 路好: 丙;\n#连线#\n爱情: 甲->乙, 乙->甲;\n友情: 甲-丙;",
      // ⚠️ 提示文案若写成多行，必须用反引号（模板字符串）包裹；用双引号跨行 = 语法错误会整站崩
      ed_link_help: `五段语法（段头用 <code>#…#</code>，可只写需要的段）：<br>
<code>#布局#</code> <code>1: 甲,乙,丙</code>（<code>0:</code>=圆心；圈上按书写顺序顺时针摆放；<code>1(8):</code>=声明 8 槽；<code>1[150px]:</code>=指定半径）<br>
<code>#喜好#</code> <code>本命: 甲,乙</code>（涂角色圆圈，一个角色一档）<br>
<code>#连线#</code> <code>爱情: 甲-&gt;乙</code>（类型词: 人物对；<code>-</code>无向 / <code>-&gt;</code>单向 / <code>&lt;-&gt;</code>双向）<br>
粗线词（本命/很喜欢/路好/不吃）＝有多喜欢；细线词（爱情/友情/亲情/QPR）＝是什么关系；同一对可叠加（既粗又细）。<br>
<code>#样式#</code> <code>敌对: #7B2FBE, 2px, top</code>（自定义颜色或新增类型词）<br>
名字含逗号、冒号、横线等符号时用引号包住：<code>'Emet-Selch'</code>。<br>
旧的夹行写法（甲—本命+爱情—乙）也兼容。<br>
<b>纯名单</b>请到「批量录入」Tab。`,

      // ---------------- 统一编辑器：数据包（导出 / 导入） ----------------
      ed_data_import_title: "导入数据包",
      ed_data_import_hint: "粘贴整份数据串（新版压缩串带头像、旧版 JSON 也认）。",
      ed_data_import_ph: "在这里粘贴数据串",
      ed_data_import_help: "把别人给你的整串数据粘进来，点「上传」。<br>新版以 <code>XHS2:</code> 开头，旧版是 JSON，两种都能读。<br>导入会覆盖当前画板全部内容。",
      ed_data_export_title: "导出数据包",
      ed_data_export_hint: "长按输入框手动全选复制。带头像（新版压缩串）。",
      ed_data_export_help: "这串文字包含人物、连线、布局、图例、头像的全部信息。<br>复制后发给别人，对方在「导入数据包」粘贴即可还原。<br>新版以 <code>XHS2:</code> 开头（带头像），旧版是 JSON（不带头像）。",
    },
